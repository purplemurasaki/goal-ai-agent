...
DB 設計の入力とする。物理テーブルは [次工程: DB設計書] で定義する。
...
| 項目 | 担当工程 |
|------|----------|
| ER 図・テーブル定義・RLS ポリシー | DB設計書の作成 |
| API エンドポイント・ディレクトリ構成 | 技術スタックの具体化 |
| OpenAI モデル選定・プロンプトテンプレート | 技術スタック / バックエンド実装 |
| 1 日あたり AI リクエスト上限の実装方式 | バックエンド実装 |
| テストライブラリ・カバレッジ方針 | 技術スタックの具体化 |
